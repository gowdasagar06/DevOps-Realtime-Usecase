...
variable "artifact_bucket"{
  type    = string
  default = "usecase-artifact-bucket123-pre-prod"
}
variable "pipeline-name"{
  type    = string
  default = "eks-terraform-codepipeline-pre-prod"
}
variable "build-plan-name"{
  type    = string
  default = "eks-deploy-plan-pre-prod"
}
variable "build-apply-name"{
  type    = string
  default = "eks-deploy-apply-pre-prod"
}
variable "codebuild_service_role-name"{
  type    = string
  default = "codebuild_service-eks-pre-prod"
}
variable "codepipeline_service_role-name"{
  type    = string
  default = "codepipeline_service-eks-pre-prod"
}
variable "buildspec_path_apply"{
  type    = string
  default = "pre-prod/buildspec-apply.yml"
}
